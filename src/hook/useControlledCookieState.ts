...
export const useControlledCookieState = <T = string>(key: string, init: T): [T, ((value: (((prevState: T) => T) | T)) => void)] => {
  ...
  const onChange = <T>(args: ((prevState: T) => T) | T) => {
    ...
    let opts: CookieSerializeOptions | undefined = result
      // по дефолту хранятся сутки
      ? {maxAge: 60 * 60 * 24}
      // в случае удаления куков, просто делаю его протухшим
      : {maxAge: -99999};

    document.cookie = cookie.serialize(key, result as unknown as string, opts);
  };
